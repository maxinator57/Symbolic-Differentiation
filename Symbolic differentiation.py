    ...
    def __init__(self, top, btm):
        ...
        if len(t) == 0:
            ...
            for i in range(2, len(t)):
                gcd = Euklid(gcd, t[i])
        b += [1, 1]
        lcm = b[0] * b[1] // Euklid(b[0], b[1])
        for i in range(2, len(b)):
            lcm = lcm * b[i] // Euklid(lcm, b[i])
...
class power:
    ...
    def derive(self):
        base = self.base
        pw = self.power
        return add(multiply(multiply(power_f(base, pw), ln(base)), pw.derive()), multiply(pw, multiply(power_f(base, sub(pw, polynomial([1], [1]))), base.derive())))
    ...
    def ptf(self):
        ...
        if mark == '+':
            ...
            if (type(f2) == mixed_func and f2.mark in ['+', '-']) or (type(f2) == polynomial and f2.btm == [1]):
                ...
                line2 = f2.ptf()
            return line + '*' + line2
        if type(f1) == trig or type(f1) == ln or type(f1) == log or type(f1) == power or (type(f1) == mixed_func and f1.mark == '*'):
            line = f1.ptf()
        else:
            line = '(' + f1.ptf() + ')'
        return line + '/' + '(' + f2.ptf() + ')'
...
def eq(f1, f2):
    if type(f1) == type(f2):
        ...
        if t == polynomial:
            ...
            if f1.mark == f2.mark and ((eq(f1.f1, f2.f1) and eq(f1.f2, f2.f2)) or (f1.mark in ['+', '*'] and (eq(f1.f2, f2.f1) and eq(f1.f1, f2.f2)))):
                return True
            return False
    return False   
...
def simplify(line):
    ...
    for i in range(len(line)):
        if line[i] == '+' and line[i + 1] in sgns:
...
while True:
    print('')
    print('Please, type your function:')
    exp = input()
    print('')
    F = True
    f = count(parse((exp)))
    if F:
        print('You have typed function f(x) =', simplify(f.ptf()))
        print('')
        result = simplify(count(f.derive()).ptf())
        print('df/dx =', result)